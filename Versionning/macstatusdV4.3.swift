...
// v4.3.4 — logique ON/OFF déterministe (locked + unlockInProgress + displaySleep) sur squelette 3.4.7
...
// MARK: - loginwindow backend (hooks 4.3.4)

func loginwindow_unlockInProgress() -> Bool {
    // TODO: implémenter via ton watcher loginwindow (unlockInProgress=true quand overlay / champ visibles)
    return false
}

func loginwindow_authUIActive() -> Bool {
    // TODO: implémenter si besoin pour debug (authUI=true = loginwindow existe, préparée ou visible)
    return false
}

func loginwindow_statusUIVisible() -> Bool {
    // TODO: implémenter si besoin pour debug (statusUI=true = overlay sans champ, statusUI=false = champ visible)
    return false
}
...
// MARK: - Effective State (ON/OFF) — 4.3.4
...
func effectiveState() -> String {
    ...
    // 1. Extinction dure : sommeil système ou écran éteint
    if asleep || displayAsleep {
        return "0"   // OFF
    }

    // 2. Bureau : locked == false → ON
    if !locked {
        return "1"   // ON
    }
    ...
    // 3.a Overlay / champ visible : unlockInProgress == true → ON
    if unlockInProgress {
        return "1"   // ON
    }

    // 3.b Screensaver visuellement seul : locked == true && unlockInProgress == false → OFF
    return "0"       // OFF
}
