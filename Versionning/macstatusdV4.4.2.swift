...
func effectiveState() -> String {
    ...
    // ON si transition de verrouillage (évite glitch OFF→ON)
...
Timer.scheduledTimer(withTimeInterval: 0.1, repeats: true) { _ in
    ...
    if locked && !unlockInProgress && !screenSaverActive && lockTransition == false {
        lockTransition = true
    ...
    if locked && screenSaverActive && unlockInProgress == false {
        lockTransition = false
    }
...
print("macstatusd 4.4.2 started.")
