...
func sendWebhook(state: Bool) {
    ...
    print("sendWebhook CALLED with state =", value)
    print("Webhook URL:", url.absoluteString)
    ...
    DispatchQueue.global().async {
        ...
        let task = URLSession.shared.dataTask(with: request) { data, response, error in
            print("Webhook RESPONSE:", response as Any)
            print("Webhook ERROR:", error as Any)
        }
        task.resume()
    }
}
...
func refreshStateAndNotify() {
    ...
    if current != lastEffectiveState {
        ...
        // Push vers Homebridge (conversion String -> Bool)
        sendWebhook(state: current == "1")
        print("MacAgent: effective state changed -> \(current)")
    }
}
