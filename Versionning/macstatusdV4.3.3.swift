...
// loginwindow-derived flags (via log stream)
...
// v4.3.7 — wake + idle logic from 3.4.7
...
// MARK: - Loginwindow watcher (log stream)
...
DispatchQueue.global().async {
    while true {
        ...
        if line.contains("shieldWindowRaised")
            ...
            shieldRaised = true
            debug("loginwindow → shieldRaised = true")
        }
        ...
        if line.contains("set password field placeholder")
            ...
            authUIActive = true
            debug("loginwindow → authUIActive = true")
        }
        ...
        if line.contains("WiFi viewDidLoad")
            ...
            debug("loginwindow → statusUIVisible = true")
        }
        ...
        if line.contains("kLWLockFromScreenSaverOtherLaunch")
            ...
            debug("loginwindow → lockRequestedByScreensaver = true")
        }
    }
}
...
// MARK: - Effective State (ON/OFF) — logique 4.3.7
...
func effectiveState() -> String {
    ...
    let unlocking = unlockInProgress
    ...
    // 3.a Screensaver seul (locked + screensaver + pas d'unlock) → OFF
    if locked && saver && !unlocking {
        return "0"
    }

    // 3.b Unlock en cours OU overlay/champ (locked + !saver OU unlocking) → ON
    if unlocking || !saver {
        return "1"
    }
...
Timer.scheduledTimer(withTimeInterval: 0.1, repeats: true) { _ in
    ...
    // Reset loginwindow flags quand on est clairement en "screensaver seul"
    if locked && saver && !unlockInProgress {
        if shieldRaised || authUIActive || statusUIVisible || lockRequestedByScreensaver {
            debug("Reset loginwindow flags (screensaver-only state)")
        }
        shieldRaised = false
        authUIActive = false
        statusUIVisible = false
        lockRequestedByScreensaver = false
        // unlockInProgress est déjà false par condition
    }
    ...
    let line = "[macstatusd 4.3.7] State: \(state) | Locked: \(locked) | Screensaver: \(saver) | Asleep: \(asleep) | displaySleep: \(displayAsleep != 0) | unlockInProgress: \(unlockInProgress) | shield: \(shieldRaised) | authUI: \(authUIActive) | statusUI: \(statusUIVisible)  \(frame)"
...
print("macstatusd 4.3.7 started.")
