...
// v3.4.6 — wake + post-interaction + polling écran + resync display wake + cycles infinis
...
// MARK: - v3.4.6 Wake + Idle Timeout + Polling
...
ws.addObserver(forName: NSWorkspace.didWakeNotification,
               object: nil, queue: .main) { _ in
    // Wake “fort” → on nettoie tout et on force ON
    stopIdleMonitoring()
    isAsleep = false
    wakePending = true
...
Timer.scheduledTimer(withTimeInterval: 0.1, repeats: true) { _ in
    ...
    // Display wake après pseudo-sleep : écran rallumé alors qu'on croit encore être asleep
    if isAsleep && displayAsleep == 0 {
        debug("Display wake détecté avec isAsleep=true → resync ON + réarmement idleTimeout")
        isAsleep = false
        stopIdleMonitoring()
        forceRefreshState()
        // On re-rentre dans une fenêtre où le Mac peut replonger en pseudo-sleep
        if locked {
            scheduleIdleTimeout()
        }
    }

    // Unlock pendant wakePending → on annule juste le scénario edge-case
    ...
    // Activité HID sur loginwindow : écran allumé + verrouillé → on réarme idleTimeout
