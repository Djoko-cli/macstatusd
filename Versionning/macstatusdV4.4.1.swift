...
// Seul oracle de l’UI loginwindow
var unlockInProgress = false

// v4.4.1 — wake + idle logic from 3.4.7
...
// MARK: - Loginwindow watcher (log stream, flag persistant)
...
// MARK: - Effective State (ON/OFF) — logique 4.4.1
...
func effectiveState() -> String {
    ...
    // ON si pipeline d’unlock actif (overlay / champ / transition)
    if unlockInProgress {
        return "1"
    }
...
Timer.scheduledTimer(withTimeInterval: 0.1, repeats: true) { _ in
    ...
    // Reset unlockInProgress quand on est clairement revenu au bureau
    if !locked && unlockInProgress {
        debug("Back to desktop → unlockInProgress = false")
    ...
    let line = "[macstatusd 4.4.1] State: \(state) | Locked: \(locked) | Screensaver: \(saver) | Asleep: \(asleep) | displaySleep: \(displayAsleep != 0) | unlockInProgress: \(unlockInProgress)  \(frame)"
...
print("macstatusd 4.4.1 started.")
