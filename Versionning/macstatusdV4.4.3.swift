...
// Wake/idle (placeholder, si tu veux les réintégrer ensuite)
...
func effectiveState() -> String {
    ...
    let overlayVisible = shieldRaised || authUIActive || statusUIVisible
    ...
    // ON si overlay visible
    if overlayVisible { return "1" }

    // ON si pipeline d’unlock actif
    if unlockInProgress { return "1" }

    // ON si transition de verrouillage (anti‑glitch)
    if lockTransition { return "1" }

    // OFF si verrouillé sans overlay ni pipeline
    return "0"
}
...
Timer.scheduledTimer(withTimeInterval: 0.1, repeats: true) { _ in
    ...
    // ESC / retour screensaver seul : locked + screensaver + pas d’overlay + pas d’unlock
...
print("macstatusd 4.4.3 started.")
