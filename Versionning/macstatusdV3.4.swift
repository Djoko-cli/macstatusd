...
// v3.4.2 — gestion du wake silencieux
    ...
    wakeTimeoutTimer = Timer.scheduledTimer(withTimeInterval: 30.5, repeats: false) { _ in
        ...
        if wakePending && isScreenLocked() && !isAsleep {
            ...
            if displayAsleep != 0 {
                ...
                // FAUX edge case : activité HID, l’écran est resté allumé
                debug("Wake timeout → écran allumé → annulation (activité HID)")
                wakePending = false
            }
            
        } else {
            // Timer devenu inutile (unlock, sleep normal, screensaver…)
...
// MARK: - Status Bar + détection unlock
...
Timer.scheduledTimer(withTimeInterval: 0.1, repeats: true) { _ in
    ...
    // v3.4.2 — si l’utilisateur déverrouille, on annule le wakePending
