...
// v4.3.5 — wake + idle logic from 3.4.7
...
// MARK: - Effective State (ON/OFF) — logique 4.3.5
...
func effectiveState() -> String {
    ...
    // 3.a Unlock en cours (overlay / champ / pipeline d’unlock)
    if unlocking {
        return "1"   // ON
    }

    // 3.b Verrouillé sans unlock → OFF
    return "0"
}
...
Timer.scheduledTimer(withTimeInterval: 0.1, repeats: true) { _ in
    ...
    let line = "[macstatusd 4.3.5] State: \(state) | Locked: \(locked) | Screensaver: \(saver) | Asleep: \(asleep) | displaySleep: \(displayAsleep != 0) | unlockInProgress: \(unlockInProgress) | shield: \(shieldRaised) | authUI: \(authUIActive) | statusUI: \(statusUIVisible)  \(frame)"
...
print("macstatusd 4.3.5 started.")
