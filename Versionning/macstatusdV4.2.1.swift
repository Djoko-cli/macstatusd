...
var authUIActive = false          // préparation interne
var authUIVisible = false         // UI réellement affichée
...
func deriveState(locked: Bool) -> MacState {
    ...
    // 1. loginwindow superposée au screensaver (TON état unique)
...
func effectiveState() -> String {
    ...
    debug("deriveState → \(state.rawValue) (locked=\(locked), saver=\(screenSaverActive), asleep=\(isAsleep), authVisible=\(authUIVisible))")
...
Timer.scheduledTimer(withTimeInterval: 0.5, repeats: true) { _ in
    ...
    let line = "[macstatusd 4.3.1] State: \(state.rawValue) | Locked: \(locked) | Screensaver: \(screenSaverActive) | Asleep: \(isAsleep) | authVisible: \(authUIVisible)  \(frame)"
    FileHandle.standardOutput.write(Data("\r\(line)".utf8))
}

// MARK: - Start

print("macstatusd 4.3.1 started.")
