...
// v3.4.7 → v3.4.8 — wake + post-interaction + polling écran + resync display wake + cycles infinis + config JSON générique + HybridSecureMode
...
func effectiveState() -> String {
    ...
    // v3.4.8 — Hybrid Secure Mode:
    // loginwindow superposée au screensaver, machine réveillée, session user toujours là.
    // Visuellement : Mac AWAKE + LOCKED → on force ON.
    if !asleep && saver && locked {
        debug("HybridSecureMode → AWAKE + LOCKED (loginwindow over ScreenSaver) → ON")
        return "1"
    }
...
func refreshStateAndNotify() {
    ...
    if current != lastEffectiveState {
        ...
        debug("State changed → \(current) (asleep=\(isAsleep), screensaver=\(screenSaverActive), locked=\(isScreenLocked()))")
    }
}
