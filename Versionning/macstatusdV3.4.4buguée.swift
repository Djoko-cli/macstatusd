...
// v3.4.4 — wake + post-interaction + polling écran + resync unlock
...
// MARK: - v3.4.4 Wake + Idle Timeout + Polling
...
// MARK: - Status Bar + détection unlock + HID
...
Timer.scheduledTimer(withTimeInterval: 0.1, repeats: true) { _ in
    ...
    // Resync critique : si on est UNLOCKED et isAsleep == true → on repasse ON
    if !locked && isAsleep {
        debug("Unlock avec isAsleep=true → resync ON")
        isAsleep = false
        stopIdleMonitoring()
        forceRefreshState()
    }

    // Unlock pendant wakePending → on annule le scénario edge-case
    ...
    // Activité HID sur loginwindow : écran allumé + verrouillé
